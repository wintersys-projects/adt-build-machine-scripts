...
then
        ...
        do
                php_module_list="${php_module_list} php${PHP_VERSION}-${php_module}"
        done
fi
...
then
        ...
        /bin/sed -i "s;XXXXPHP_MODULESXXXX;${php_module_list};" ${BUILD_HOME}/runtimedata/${CLOUDHOST}/${BUILD_IDENTIFIER}/cloud-init/webserver.yaml
