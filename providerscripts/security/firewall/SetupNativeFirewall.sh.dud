...
then
	...
	then       
		...
		then
			...
			if ( [ "${BUILD_MACHINE_VPC}" = "0" ] )
			then
				/usr/local/bin/linode-cli firewalls rules-update  --inbound_policy DROP   --outbound_policy ACCEPT --inbound  '[{"addresses":{"ipv4":["'${VPC_IP_RANGE}'"]},"action":"ACCEPT","protocol":"TCP","ports":"1-65535"},{"addresses":{"ipv4":["'${build_machine_ip}/32'"]},"action":"ACCEPT","protocol":"TCP","ports":"'${SSH_PORT}'"},{"addresses":{"ipv4":["0.0.0.0/0"]},"action":"ACCEPT","protocol":"ICMP"}]' ${autoscaler_firewall_id}
			elif ( [ "${BUILD_MACHINE_VPC}" = "1" ] )
			then
				/usr/local/bin/linode-cli firewalls rules-update  --inbound_policy DROP   --outbound_policy ACCEPT --inbound  '[{"addresses":{"ipv4":["'${VPC_IP_RANGE}'"]},"action":"ACCEPT","protocol":"TCP","ports":"1-65535"},{"addresses":{"ipv4":["0.0.0.0/0"]},"action":"ACCEPT","protocol":"ICMP"}]' ${autoscaler_firewall_id}
			fi
			...
			then
				/usr/local/bin/linode-cli firewalls rules-update  --inbound_policy DROP   --outbound_policy ACCEPT --inbound  '[{"addresses":{"ipv4":["'${VPC_IP_RANGE}'"]},"action":"ACCEPT","protocol":"TCP","ports":"1-65535"},{"addresses":{"ipv4":["0.0.0.0/0"]},"action":"ACCEPT","protocol":"ICMP"}]' ${database_firewall_id}
			fi
		...
		then
			...
			if ( [ "`/usr/local/bin/linode-cli --json firewalls list | /usr/bin/jq -r '.[] | select (.label == "adt-autoscaler-'${BUILD_IDENTIFIER}'").id'`" = "" ] )
			then
				/usr/local/bin/linode-cli firewalls create --label "adt-autoscaler-${BUILD_IDENTIFIER}" --rules.inbound_policy ACCEPT   --rules.outbound_policy ACCEPT
			fi
                        
			if ( [ "`/usr/local/bin/linode-cli --json firewalls list | /usr/bin/jq -r '.[] | select (.label == "adt-webserver-'${BUILD_IDENTIFIER}'").id'`" = "" ] )
			then
    			/usr/local/bin/linode-cli firewalls create --label "adt-webserver-${BUILD_IDENTIFIER}" --rules.inbound_policy ACCEPT   --rules.outbound_policy ACCEPT
			fi
                        
			if ( [ "`/usr/local/bin/linode-cli --json firewalls list | /usr/bin/jq -r '.[] | select (.label == "adt-database-'${BUILD_IDENTIFIER}'").id'`" = "" ] )
			then
				/usr/local/bin/linode-cli firewalls create --label "adt-database-${BUILD_IDENTIFIER}" --rules.inbound_policy ACCEPT   --rules.outbound_policy ACCEPT
			fi
        fi    
    fi
