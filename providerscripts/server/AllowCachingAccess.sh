...
if ( [ "${CLOUDHOST}" = "aws" ] )
then
    if ( [ "${IN_MEMORY_CACHING}" != "" ] && [ "${IN_MEMORY_SECURITY_GROUP}" != "" ] )
    then
        /usr/bin/aws ec2 authorize-security-group-ingress --group-id ${IN_MEMORY_SECURITY_GROUP} --protocol tcp --port ${IN_MEMORY_CACHING_PORT} --cidr ${IP_TO_ALLOW}/32
    fi
fi