...
then
	...
	# historical : /usr/local/bin/linode-cli linodes create --authorized_keys "${key}" --root_pass "${emergency_password}" --region ${REGION} --image "${OS_CHOICE}" --type ${server_size} --label "${server_name}" --no-defaults --interfaces.primary true --interfaces.purpose vpc --interfaces.subnet_id ${subnet_id} --interfaces.ipv4.nat_1_1 any --metadata.user_data "${cloud_config}" --disk_encryption "enabled"
	# legacy compatible : /usr/local/bin/linode-cli linodes create --authorized_keys "${key}" --root_pass "${emergency_password}" --region ${REGION} --image "${OS_CHOICE}" --type ${server_size} --label "${server_name}" --no-defaults --interface_generation "legacy_config" --interfaces '[ { "primary": true, "purpose": "vpc", "subnet_id": '${subnet_id}',  "ipv4": { "nat_1_1": "any" } } ]' --metadata.user_data "${cloud_config}" --disk_encryption "enabled"
	/usr/local/bin/linode-cli linodes create --authorized_keys "${key}" --root_pass "${emergency_password}" --region ${REGION} --image "${OS_CHOICE}" --type ${server_size} --label "${server_name}" --no-defaults --interface_generation "linode" --interfaces ' [ { "purpose": "public", "firewall_id": "'${firewall_id}'", "default_route": { "ipv4": true }, "public": { "ipv4": { "addresses": [ { "address": "auto", "primary": true } ] } }},   { "purpose": "vpc", "default_route": { "ipv4": true },  "firewall_id": "'${firewall_id}'", "vpc": { "ipv4": { "addresses": [ { "address": "auto", "primary": false } ], "ranges": [ { "range": null } ] }} , "subnet_id": '${subnet_id}'  } ]' --metadata.user_data "${cloud_config}" --disk_encryption "enabled"
fi
