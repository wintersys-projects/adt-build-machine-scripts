...
then
        ...
        datastore_cmd="${datastore_tool} --recursive --force  --config=/root/.s3cfg-${count} --host=https://${host_base} del s3://${config_bucket}/"
elif ( [ "${datastore_tool}" = "/usr/bin/s5cmd" ] )
then
        host_base="`/bin/grep host_base /root/.s5cfg-${count} | /usr/bin/awk -F'=' '{print  $NF}' | /bin/sed 's/ //g'`"
        datastore_cmd="/usr/bin/s5cmd --credentials-file /root/.s5cfg-${count} --endpoint-url https://${host_base} rm s3://${config_bucket}/"
elif ( [ "${datastore_tool}" = "/usr/bin/rclone" ] )
then
        datastore_cmd="${datastore_tool} --config /root/.config/rclone/rclone.conf-${count} purge s3:${config_bucket}/"
fi
