...
# Author: Peter Winter
# Date :  24/02/2022
# Description: This will create a particular bucket in our config datastore
#######################################################################################
# License Agreement:
...
then
	datastore_cmd="${datastore_tool} mb "
elif ( [ "${datastore_tool}" = "/usr/bin/s5cmd" ] )
then
	host_base="`/bin/grep host_base /root/.s5cfg | /bin/grep host_base | /usr/bin/awk -F'=' '{print  $NF}' | /bin/sed 's/ //g'`" 
	datastore_cmd="${datastore_tool} --credentials-file /root/.s5cfg --endpoint-url https://${host_base} mb "
fi
	
 ${datastore_cmd} s3://${configbucket}
