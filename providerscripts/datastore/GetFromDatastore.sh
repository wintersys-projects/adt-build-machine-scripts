...
then
        ...
        host_base="`/bin/grep host_base ${config_file} | /usr/bin/awk -F'=' '{print  $NF}' | /bin/sed 's/ //g' | /usr/bin/head -1`" 
        datastore_cmd="${datastore_tool} --config=${config_file} --force --recursive --host=https://${host_base} get s3://"
