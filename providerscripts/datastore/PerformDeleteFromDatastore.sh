...
# Author: Peter Winter
# Date :  9/4/2016
# Description: Actually delte file(s) from a datastore bucket with the current S3 provider and 
# using the tool that is currently configured as active
##########################################################################################
# License Agreement:
...
then
        ...
        datastore_cmd1="${datastore_tool} --config /root/.config/rclone/rclone.conf-${count} --s3-endpoint ${host_base} delete s3:"
fi
