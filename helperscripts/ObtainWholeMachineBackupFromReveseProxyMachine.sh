...
if ( [ ! -d ${BUILD_HOME}/runtimedata/wholemachinebackups/reverseproxies/${WEBSITE_URL} ] )
then
        /bin/mkdir -p ${BUILD_HOME}/runtimedata/wholemachinebackups/reverseproxies/${WEBSITE_URL}
fi

/usr/bin/scp -o ConnectTimeout=5 -o ConnectionAttempts=2 -o UserKnownHostsFile=${REVERSE_PROXY_PUBLIC_KEYS} -o StrictHostKeyChecking=yes -P ${SSH_PORT} -i ${BUILD_HOME}/runtimedata/${CLOUDHOST}/${BUILD_IDENTIFIER}/keys/id_${ALGORITHM}_AGILE_DEPLOYMENT_BUILD_KEY_${BUILD_IDENTIFIER} ${SERVER_USERNAME}@${RP_IP}:/home/${SERVER_USERNAME}/machinedump/reverseproxy_backup.tar.gz ${BUILD_HOME}/runtimedata/wholemachinebackups/reverseproxies/${WEBSITE_URL}
