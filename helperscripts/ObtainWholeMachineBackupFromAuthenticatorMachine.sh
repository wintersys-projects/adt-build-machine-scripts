...
# Author: Peter Winter
# Date  : 13/07/2016
# Description : This will obtain the tar archive of a whole authenticator machine if it available
######################################################################################################################################################
# License Agreement:
...
if ( [ ! -f  ./ObtainWholeMachineBackupFromAuthenticatorMachine.sh ] )
then
...
token_to_match="auth-`${BUILD_HOME}/helperscripts/GetVariableValue.sh REGION`-${BUILD_IDENTIFIER}"
...
if ( [ "${ips}" = "" ] )
then
        /bin/echo "There doesn't seem to be an authenticator running"
        exit
fi

DIR="`/bin/pwd`"

/bin/echo "Which Authenticator server would you like to connect to?"
count=1
for ip in ${ips}
do
        /bin/echo "${count}:   ${ip}"
...
WEBSERVER_PUBLIC_KEYS="${BUILD_HOME}/runtimedata/${CLOUDHOST}/${BUILD_IDENTIFIER}/keys/authenticator_${WEB_IP}keys"
...
/bin/echo "About to attempt a whole machine backup of a authenticator class machine. Press <enter> to have the backup process begin"
read x

/usr/bin/ssh -q -o ConnectTimeout=5 -o ConnectionAttempts=2 -o UserKnownHostsFile=${WEBSERVER_PUBLIC_KEYS} -o StrictHostKeyChecking=yes -p ${SSH_PORT} -i ${BUILD_HOME}/runtimedata/${CLOUDHOST}/${BUILD_IDENTIFIER}/keys/id_${ALGORITHM}_AGILE_DEPLOYMENT_BUILD_KEY_${BUILD_IDENTIFIER} ${SERVER_USERNAME}@${WEB_IP} "${SUDO} /home/${SERVER_USERNAME}/utilities/housekeeping/GenerateWholeMachineBackup.sh"

if ( [ ! -d ${BUILD_HOME}/runtimedata/wholemachinebackups/${WEBSITE_URL}/authenticator ] )
then
        /bin/mkdir -p ${BUILD_HOME}/runtimedata/wholemachinebackups/${WEBSITE_URL}/authenticator
fi

if ( [ ! -f ${BUILD_HOME}/runtimedata/wholemachinebackups/${WEBSITE_URL}/credentials.dat ] )
then
        /usr/bin/scp -o ConnectTimeout=5 -o ConnectionAttempts=2 -o UserKnownHostsFile=${WEBSERVER_PUBLIC_KEYS} -o StrictHostKeyChecking=yes -P ${SSH_PORT} -i ${BUILD_HOME}/runtimedata/${CLOUDHOST}/${BUILD_IDENTIFIER}/keys/id_${ALGORITHM}_AGILE_DEPLOYMENT_BUILD_KEY_${BUILD_IDENTIFIER} ${SERVER_USERNAME}@${WEB_IP}:/home/${SERVER_USERNAME}/machinedump/authenticator_credentials.dat ${BUILD_HOME}/runtimedata/wholemachinebackups/${WEBSITE_URL}
fi
...
/usr/bin/tar tvf ${BUILD_HOME}/runtimedata/wholemachinebackups/${WEBSITE_URL}/authenticator/authenticator_backup.tar 2>&1 > /dev/null
