...
then
        datastorebucket="0"
        datastoredbbucket="0"
        ${BUILD_HOME}/providerscripts/datastore/operations/GetFromDatastore.sh "backup-web" "applicationsourcecode.tar.gz" "." "${BUILD_ARCHIVE_CHOICE}"
        ${BUILD_HOME}/providerscripts/datastore/operations/GetFromDatastore.sh "backup-db" "${WEBSITE_NAME}-DB-backup.tar.gz" "." "${BUILD_ARCHIVE_CHOICE}"
...
if ( [ "${gitrepo}" = "1" ] )
then
        ${BUILD_HOME}/providerscripts/git/GitClone.sh ${APPLICATION_REPOSITORY_PROVIDER} ${APPLICATION_REPOSITORY_USERNAME} ${APPLICATION_REPOSITORY_OWNER} ${APPLICATION_BASELINE_SOURCECODE_REPOSITORY} ${APPLICATION_REPOSITORY_TOKEN}
        ${BUILD_HOME}/providerscripts/git/GitClone.sh ${APPLICATION_REPOSITORY_PROVIDER} ${APPLICATION_REPOSITORY_USERNAME} ${APPLICATION_REPOSITORY_OWNER} ${BASELINE_DB_REPOSITORY} ${APPLICATION_REPOSITORY_TOKEN}
